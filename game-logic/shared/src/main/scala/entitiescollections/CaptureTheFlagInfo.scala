...
import gamelogic.entities.concreteentities.{GameArea, Zone}
...
final class CaptureTheFlagInfo(playerInfo: Seq[CaptureTheFlagInfo.PlayerInfo]) {
  ...
  val teamsByPlayerId: Map[Entity.Id, PlayerTeam] = teams.flatMap(elem => elem._2.playerIds.map(_ -> elem._2))
  ...
  val popPositionsByPlayerId: Map[Entity.Id, Complex] = playerInfo.map(info => info.id -> popPositions(info.team)).toMap

}

object CaptureTheFlagInfo {

  final case class PlayerInfo(playerName: String, abilities: List[Int], team: Int, id: Entity.Id)
