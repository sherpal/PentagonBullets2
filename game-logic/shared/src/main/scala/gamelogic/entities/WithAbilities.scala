...
/** A [[WithAbilities]] [[Entity]] have a position because we need to check that they don't move while casting. Of
  * course, something casting but without actual position could "fake" one.
  */
trait WithAbilities extends WithPosition {
  ...
  def removeRelevantAbility(useId: UseId, time: Long): WithAbilities
  ...
  /** This Map remembers the last usage of each of the abilities that this [[WithAbilities]] has. This probably need a
    * little bit more thought, but the idea is that, in the future of the game, all knowledge about what the next
    * ability can do, or may be used, or anything else.
    */
  val relevantUsedAbilities: Map[Ability.UseId, Ability]

  /** Returns an error message when this [[WithAbilities]] can not cast this ability. Returns None if it can.
    *
    * @param ability
