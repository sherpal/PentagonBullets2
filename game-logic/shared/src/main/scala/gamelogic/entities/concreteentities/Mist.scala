...
/** A (the?) Mist is similar to a DamageZone, but its purpose and shape is different.
  *
  * The Mist is a neighbourhood of the boundary of the game, and progressively grows until only the two players square
  * is not covered.. This forces players to center as the game continues.
  *
  * At each tick, every player in the Mist takes some damage.
  */
final case class Mist(id: Entity.Id, lastGrow: Long, lastTick: Long, shape: Polygon) extends Body {
...
object Mist {
  ...
  /** Sets the value of _sideShrinking.
    * @param maxGameSide
    *   size of a side of the GameArea at the beginning of the game. This is also the size of the mist at the beginning
    * @param nbrPlayers
  ...
  /** Compute the value of the mist side length when current time is time.
    ...
    * reaches the full size, the length is given by f(t) = (l - L) * t^2 / M^2 + L
    */
