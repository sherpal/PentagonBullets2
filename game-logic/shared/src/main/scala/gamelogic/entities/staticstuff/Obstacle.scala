...
/** An [[Obstacle]] is a fixed entity on the ground that basically does nothing, except keeping other entities from
  * going through it. Also, it has its importance for spells with a target. When there is an obstacle between two
  * entities, one can not use an ability which targets the other one.
  */
...
object Obstacle {
  ...
  /** Generates positively oriented vertices for a rectangle roughly equivalent to a segment from `z1` to `z2`, with the
    * given `thickness`.
    */
  ...
  /** Creates an [[Obstacle]] which is roughly a "segment" between the points `z1` and `z2`. The `thickness` parameter
    * determines how thick the obstacle will be. Something like 5 (in current game coordinate system) is good for a
    * "thin" obstacle.
    */
  def segmentObstacle(
      id: Entity.Id,
