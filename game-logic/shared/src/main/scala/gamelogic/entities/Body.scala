...
trait Body extends WithPosition {
  ...
  /** Checks if this MovingBody collides with that Moving body.
    ...
    *   other body
    * @param currentTime
    *   current time to check collision
    */
  ...
  /** Finds the first position, going from pos and towards rotation, where this body does not collide the bodies.
    *
    * @param rotation
    *   The angle towards which we need to go.
    * @param bodies
    *   All the bodies we want to avoid.
    * @param precision
    *   Steps by which trying.
    * @return
    *   First position where this body will have no collision.
    */
  ...
  /** Find the position z closest to targetPosition, on the segment [Complex(xPos, yPos), targetPosition] such that
    ...
    *   The position that we try to reach
    * @param bodies
    *   All the Bodies that can enter in collision
    * @param precision
    *   The step moving forward (typically 5-10 pixels)
    * @return
    *   The final position
    */
  def lastValidPosition(targetPosition: Complex, bodies: Iterable[Body], precision: Int = 5): Complex = {
    ...
    if (dist < 2) pos
      ...
      def tryNextPos(curPos: Complex): Complex = {
        ...
        if (
          bodies.exists(body => body.shape.collides(body.pos, body.rotation, shape, nextPos, rotation))
