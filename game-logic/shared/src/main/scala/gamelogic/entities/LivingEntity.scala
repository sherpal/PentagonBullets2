...
trait LivingEntity extends Entity {
  ...
  def changeLifeTotal(delta: Double): LivingEntity =
    patchLifeTotal((0.0 max (lifeTotal + delta)) min maxLife)

  /** Boolean indicating whether this [[Entity]] is normally affected by "stun" ability.
    *
    * Stunning an [[Entity]] is a very powerful ability and therefore sometimes it can not be used, otherwise it would
    * be too powerful. Typically, bosses are *not* affected by such mechanism.
    */
  def canBeStunned: Boolean = false

  /** Copy this entity by only changing its life total.
    *
    * Note: this method is not supposed to take care about clamping the new life total to [0, maxLife], but must simply
    * patch the value as is.
    *
    * Typically, this will be implemented in concrete sub case classes as `copy(lifeTotal = newLife)`.
    */
