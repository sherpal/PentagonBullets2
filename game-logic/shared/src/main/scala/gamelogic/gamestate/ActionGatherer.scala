...
trait ActionGatherer {
  ...
  /** Blindly add the actions, then remove all actions with ids in the list of ids to remove.
    *
    * This method should be consistent with, but more efficient than, the `masterAddAndRemoveActions`. The property that
    * must always hold is the following: Given actions a_1, ..., a_n, and an action collector A, val (next, oldestTime,
    * toRemove) = A.masterAddAndRemoveActions(List(a_1, ..., a_n)) then next must be equivalent to
  ...
  /** Adds new actions and remove then-illegal ones. See `slaveAddAndRemoveActions` to see what property these functions
    * must hold.
    *
    * @param actionsToAdd
    *   new actions for the [[gamelogic.gamestate.GameState]]
    * @return
    *   a triplet with the new action collector, the oldest time within actions to remove, and ids of actions to remove.
    */
