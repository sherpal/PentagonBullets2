...
final class Camera(canvas: html.Canvas) {
  ...
  /** Transform a coordinate where top right of canvas is 0 and imaginary increase to the bottom into the corresponding
    * coordinate where the center of canvas is 0 and imaginary increase to the top.
    */
  ...
  /** Change coordinates from world position to mouse position. This may be used for the GUI Regions.
    */
  ...
  /** Gives the visibility, the position and the scales of the sprite.
    *
    * @param sprite
    *   Sprite to give the spec to.
    * @param worldPos
    *   The position in world coordinates at which the sprite must be anchored.
    * @param boundingBox
    *   The boundingBox of the shape associated to the sprite, in world coordinates. The center of the bounding box must
    *   be worldPos.
    */
  ...
  /** Gives the visibility, the position and the scales of the sprite.
    *
    * @param container
    *   [[typings.pixiJs.mod.Container]] to give the spec to.
    * @param worldPos
    *   The position in world coordinates at which the sprite must be anchored.
    * @param boundingBoxCenter
    *   Center of the boundingBox in world coordinates.
    * @param boundingBox
    *   The boundingBox of the shape associated to the sprite, in world coordinates. The center of the bounding box must
    *   be worldPos.
    */
  ...
  def viewportManager(spritesWithShapes: Iterable[(Sprite, Complex, BoundingBox)]): Unit =
    spritesWithShapes.foreach { case (sprite, worldPos, boundingBox) =>
      viewportManager(sprite, worldPos, boundingBox)
    }
