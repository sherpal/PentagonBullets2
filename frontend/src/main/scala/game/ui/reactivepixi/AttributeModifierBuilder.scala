...
/** An [[AttributeModifierBuilder]] is the base trait for creating simple [[PixiModifier]] modifying attributes of
  ...
  *
  * @tparam El
  *   type of [[ReactivePixiElement]] that this [[AttributeModifierBuilder]] creates [[PixiModifier]] for. For example,
  *   a `AttributeModifierBuilder[ReactivePixiContainer, _]` creates modifiers for reactive pixi containers.
  * @tparam A
  *   type of the attribute being affected. For example, [[java.lang.Double]] for `x`, [[utils.misc.Colour]] for `tint`.
  *   The type is sometimes a little bit more involved than the js type, in order to increase type safety. However, a
  *   more "close to the metals" alternative should always exist.
  */
trait AttributeModifierBuilder[-El <: ReactivePixiElement.Base, A] {
  ...
  def <--(as: Observable[A]): Binder[El] = Binder(element => as.foreach(modifying(element, _))(element))

  def zip[El1 <: El, B](that: AttributeModifierBuilder[El1, B]): AttributeModifierBuilder[El1, (A, B)] =
    AttributeModifierBuilder.factory[El1, (A, B)] { case (element, (a, b)) =>
...
object AttributeModifierBuilder {
  ...
  final val anchorXY = factory[ReactiveSprite, (Double, Double)] { case (element, (x, y)) =>
    element.ref.anchor.set(x, y)
  }

  final val scale = factory[ReactiveContainer, Double](_.ref.scale.set(_))
  final val scaleXY = factory[ReactiveContainer, (Double, Double)] { case (element, (scaleX, scaleY)) =>
    element.ref.scale.set(scaleX, scaleY)
  }
  ...
  /** This is supposed to model a transformation that you may want to apply to your [[typings.pixiJs.PIXI.Graphics]]. As
    * such, this transformer can do anything but is primarily thought to change the geometry of the graphics.
    *
