...
/** Creates a simple effect where the given text follows the given movement in time.
  ...
  * @param path
  *   [[game.ui.effects.Path]] to describe the position of the text in function of time. The position has to be in game
...
final class SimpleTextEffect(
  ...
  val pixiText = new Text(
    text,
    new TextStyle(Align().setFontSize(fontSize).setFill(colour.rgb))
  )
