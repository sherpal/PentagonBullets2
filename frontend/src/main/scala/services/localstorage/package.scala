...
package object localstorage {
  ...
  def createKey(value: String): ZIO[LocalStorage, Nothing, Key] =
    ZIO.accessM(_.get[LocalStorage.Service].createKey(value))
  ...
  def storeAtWithEffect[A](key: Key, element: A, effect: A => Task[Unit])(implicit
      encoder: Encoder[A]
  ...
  def storeAtFor[A](key: Key, element: A, duration: Duration)(implicit
      encoder: Encoder[A]
  ): ZIO[LocalStorage, Throwable, Unit] =
    ZIO.accessM(_.get.storeAtFor(key, element, duration))

  def retrieveFrom[A](key: Key)(implicit
      decoder: Decoder[A]
