...
object LocalStorage {
  ...
  trait Service {
    ...
    def createKey(value: String): UIO[Key] = UIO(value)

    protected implicit class KeyToString(key: Key) {
    ...
    /** Store the given element at the given key, to be retrieved later.
      ...
      */
    final def storeAtFor[A](key: Key, element: A, duration: Duration)(implicit
        encoder: Encoder[A]
    ): ZIO[Any, Throwable, Unit] =
      for {
    ...
    /** Try to retrieve an element of type A from the local storage. If the element is expired as specified when it was
      ...
      */
    final def retrieveFrom[A](key: Key)(implicit
        decoder: Decoder[A]
