...
import utils.websocket.{BinaryWebSocket, Socket}
import zio.*
import zio.duration.*
...
object GamePlaying {
  ...
  def apply(gameStarted: GameStarted, stateChanger: Observer[AnyAppState]) = {
    ...
    val assetMapEvents = assetMap.signal.changes.collect { case Some(map) => map }
    ...
    val gamePlayingContainerEvents = timeDeltaChanges
      .combineWith(
        myEntityIdEvents,
        assetMapEvents
      )
    ...
    div(
      child <-- timeDeltaChanges
        ...
        .map {
          ...
          case Some((timeDelta, myEntityId, assetMap)) =>
            GamePlayingContainer(playerName, timeDelta, myEntityId, application, socket.$in, socket, assetMap)
        },
